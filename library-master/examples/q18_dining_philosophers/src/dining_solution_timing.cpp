...
/**
 * Pick up left fork then right fork if possible, then eat.  If not possible to pick up right fork, then drop left
 * and wait a random amount of time before trying again.
...
void diner_thread(int id, Fork* left, Fork* right, rendezvous* rdv) {
  ...
  while (!quit) {
    ...
    if (phil.try_pickup(right)) {
      std::cout << "Philosopher " << phil.id() << " eating." << std::endl;
      phil.eat(20);
      phil.release_forks();
    } else {
      std::cout << "  Philosopher " << phil.id() << " can't pick up right fork, dropping left" << std::endl;
      phil.release_forks();
      phil.think(std::rand() % 100); // think for random amount of time
    }
